...
func main() {
	...
	seenFileNames := map[string]map[string]bool{}
	...
	for _, dirName := range args[1:] {
		...
		for _, a := range annos {
			...
			if _, ok := seenFileNames[fn]; !ok {
				seenFileNames[fn] = map[string]bool{}
			}

			seenFileNames[fn][dirName] = true //append(seenFileNames[fn], dirName)

			if dirs := seenFileNames[fn]; len(dirs) > 1 {

				var dirs0 []string
				for k, _ := range dirs {
					dirs0 = append(dirs0, k)
				}

				msg := fmt.Sprintf("File name '%s' in multiple dirs %s", fn, strings.Join(dirs0, ", "))

				issues[dbapi.ValRes{Level: "error", Message: "duplicate_file_name" + "\t" + msg}]++
			}
